export class Time {
	 ...
	 */
	runLoop(
		update: (time: this) => void,
		render?: (time: this) => void,
	): { pause: () => void; play: () => void } | undefined {
		if (this.initialised) {
			console.warn(
				"attempt at running multiple time loops on the same timeline",
			);
			return;
		}
