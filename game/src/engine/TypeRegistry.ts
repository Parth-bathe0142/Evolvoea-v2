...
export class TypeRegistry {
    ...
    register<T extends GameObject>(
        ...
        if (this.byClass.has(cls)) {
            throw new Error(
                `Type "${cls.typeName}" is already registered`,
            );
        }
        ...
        if (existing) {
            throw new Error(
                `Type ID collision: "${cls.typeName}" and  +
                "${existing.name}" both hash to ${id}`
            );
        }
    ...
    getId<T extends GameObject>(
        cls: GameObjectClass<T>,
    ): TypeId {
        const info = this.byClass.get(cls);
        if (!info) {
            throw new Error(
                `Type "${cls.typeName}" is not registered`
            );
        }
        return info.id;
    }
    ...
    isExactType<T extends GameObject>(
        id: Id,
        cls: GameObjectClass<T>,
    ): boolean {
        return id.typeId === this.getId(cls);
    }
    isType<T extends GameObject>(
        id: Id,
        cls: GameObjectClass<T>,
    ): boolean {
        return this.getInfo(id.typeId)
            .ancestors
            .has(this.getId(cls));
    }
}